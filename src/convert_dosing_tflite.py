"""
Model B TFLite Conversion + INT8 Quantization
=================================================
Converts the trained Model B (dosing_model.keras) into TFLite format,
float32 and int8 quantized, and compares accuracy/size across:
    - Original Keras model (float32)
    - TFLite float32
    - TFLite int8 (NPU-ready)

Outputs:
    models/dosing_model_float32.tflite
    models/dosing_model_int8.tflite
    results/dosing_quantization_comparison.json

Usage:
    python3 src/convert_dosing_tflite.py
"""
...
KERAS_MODEL_PATH = os.path.join(MODELS_DIR, "dosing_model.keras")
SCALER_PATH = os.path.join(MODELS_DIR, "dosing_scaler.joblib")
TFLITE_FLOAT32_PATH = os.path.join(MODELS_DIR, "dosing_model_float32.tflite")
TFLITE_INT8_PATH = os.path.join(MODELS_DIR, "dosing_model_int8.tflite")
...
def main():
    print("Loading Model B (Keras) + scaler...")
    if not os.path.exists(KERAS_MODEL_PATH):
        raise FileNotFoundError(
            f"{KERAS_MODEL_PATH} not found. Run src/train_dosing_model.py first."
        )
    ...
    results["keras_float32"]["size_kb"] = round(os.path.getsize(KERAS_MODEL_PATH) / 1024, 2)
    ...
    results["tflite_float32"]["size_kb"] = round(os.path.getsize(TFLITE_FLOAT32_PATH) / 1024, 2)
    ...
    results["tflite_int8"]["size_kb"] = round(os.path.getsize(TFLITE_INT8_PATH) / 1024, 2)
    ...
    with open(os.path.join(RESULTS_DIR, "dosing_quantization_comparison.json"), "w") as f:
        json.dump(results, f, indent=2)

    print(f"\nSaved:")
    print(f"  - {TFLITE_FLOAT32_PATH}")
    print(f"  - {TFLITE_INT8_PATH}")
    print(f"  - {RESULTS_DIR}/dosing_quantization_comparison.json")
    print("\nDone.")
